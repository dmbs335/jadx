...
class RegionMakerTest {
	...
	@Test
	void classifyOnlyLinearConstOrMoveAsSafeRecursiveLeafDuplication() {
		BlockNode constLeaf = makeConstBlock(ArgType.INT, ArgType.INT);
		connect(constLeaf, new BlockNode(2, 0, 0));
		assertThat(RegionMaker.isSafeRecursiveLeafDuplication(constLeaf)).isTrue();

		BlockNode arithmeticLeaf = new BlockNode(3, 0, 0);
		InsnNode arithInsn = new InsnNode(InsnType.ARITH, 2);
		arithInsn.setResult(InsnArg.reg(0, ArgType.INT));
		arithInsn.addArg(InsnArg.reg(1, ArgType.INT));
		arithInsn.addArg(InsnArg.lit(1, ArgType.INT));
		arithmeticLeaf.getInstructions().add(arithInsn);
		connect(arithmeticLeaf, new BlockNode(4, 0, 0));
		assertThat(RegionMaker.isSafeRecursiveLeafDuplication(arithmeticLeaf)).isFalse();

		constLeaf.addAttr(ExpectedTraversalCycleAttr.INSTANCE);
		assertThat(RegionMaker.isSafeRecursiveLeafDuplication(constLeaf)).isFalse();

		BlockNode terminalConst = makeConstBlock(ArgType.INT, ArgType.INT);
		assertThat(RegionMaker.isSafeRecursiveLeafDuplication(terminalConst)).isFalse();
	}

	@Test
	void detectExceptionBoundaryWithinTwoEdges() {
		BlockNode leaf = makeConstBlock(ArgType.INT, ArgType.INT);
		BlockNode bridge = new BlockNode(2, 0, 0);
		BlockNode exceptionTop = new BlockNode(3, 0, 0);
		exceptionTop.add(AFlag.EXC_TOP_SPLITTER);
		connect(leaf, bridge);
		connect(bridge, exceptionTop);

		assertThat(RegionMaker.isNearExceptionBoundary(leaf)).isTrue();

		BlockNode isolatedLeaf = makeConstBlock(ArgType.INT, ArgType.INT);
		connect(isolatedLeaf, new BlockNode(4, 0, 0));
		assertThat(RegionMaker.isNearExceptionBoundary(isolatedLeaf)).isFalse();
	}
	...
	@Test
	void classifyOnlyKnownImmutableValueConstructors() {
		RootNode root = new RootNode(new JadxArgs());
		ConstructorInsn pair = makeConstructor(root, "kotlin.Pair", ArgType.OBJECT, ArgType.OBJECT);
		ConstructorInsn triple = makeConstructor(
				root, "kotlin.Triple", ArgType.OBJECT, ArgType.OBJECT, ArgType.OBJECT);
		ConstructorInsn mutable = makeConstructor(root, "java.util.ArrayList");
		ConstructorInsn wrongArity = makeConstructor(root, "kotlin.Triple", ArgType.OBJECT, ArgType.OBJECT);
		ConstructorInsn superCall = new ConstructorInsn(pair.getCallMth(), CallType.SUPER);

		assertThat(RegionMaker.isKnownImmutableValueConstructor(pair)).isTrue();
		assertThat(RegionMaker.isKnownImmutableValueConstructor(triple)).isTrue();
		assertThat(RegionMaker.isKnownImmutableValueConstructor(mutable)).isFalse();
		assertThat(RegionMaker.isKnownImmutableValueConstructor(wrongArity)).isFalse();
		assertThat(RegionMaker.isKnownImmutableValueConstructor(superCall)).isFalse();
	}
	...
	void classifyOnlyKnownPureValueCalls() {
		...
		assertThat(RegionMaker.isPureValueCall("retrofit2.Response", "headers", 1)).isTrue();
		assertThat(RegionMaker.isPureValueCall("retrofit2.Response", "code", 1)).isTrue();
		assertThat(RegionMaker.isPureValueCall("retrofit2.Response", "raw", 1)).isTrue();
		assertThat(RegionMaker.isPureValueCall("kotlin.TuplesKt", "to", 2)).isTrue();
		...
		assertThat(RegionMaker.isPureValueCall("retrofit2.Response", "code", 0)).isFalse();
		assertThat(RegionMaker.isPureValueCall("retrofit2.Response", "close", 1)).isFalse();
		assertThat(RegionMaker.isPureValueCall("kotlin.TuplesKt", "to", 1)).isFalse();
	}
	...
	@Test
	void classifyOnlyDirectRegisterThrowToMethodExit() {
		BlockNode throwBlock = new BlockNode(1, 0, 0);
		InsnNode throwInsn = new InsnNode(InsnType.THROW, 1);
		throwInsn.addArg(InsnArg.reg(0, ArgType.THROWABLE));
		throwBlock.getInstructions().add(throwInsn);
		BlockNode exit = new BlockNode(2, 0, 0);
		exit.add(AFlag.MTH_EXIT_BLOCK);
		connect(throwBlock, exit);

		assertThat(RegionMaker.isSafePlainThrowDuplication(throwBlock)).isTrue();

		throwInsn.setArg(0, InsnArg.wrapInsnIntoArg(new InsnNode(InsnType.NEW_INSTANCE, 0)));
		assertThat(RegionMaker.isSafePlainThrowDuplication(throwBlock)).isFalse();

		throwInsn.setArg(0, InsnArg.reg(0, ArgType.THROWABLE));
		exit.remove(AFlag.MTH_EXIT_BLOCK);
		assertThat(RegionMaker.isSafePlainThrowDuplication(throwBlock)).isFalse();
	}
	...
	private static ConstructorInsn makeConstructor(RootNode root, String clsName, ArgType... argTypes) {
		MethodInfo constructor = MethodInfo.fromDetails(
				root, ClassInfo.fromName(root, clsName), "<init>", List.of(argTypes), ArgType.VOID);
		ConstructorInsn ctrInsn = new ConstructorInsn(constructor, CallType.CONSTRUCTOR);
		for (int i = 0; i < argTypes.length; i++) {
			ctrInsn.addArg(InsnArg.reg(i, argTypes[i]));
		}
		return ctrInsn;
	}
