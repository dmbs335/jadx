...
class FinishTypeInferenceTest {
	...
	@Test
	void detectBooleanCoroutineSpillAcrossTwoCodeVarRelay() {
		RootNode root = new RootNode(new JadxArgs());
		ArgType stateType = ArgType.object("test.State");

		CodeVar booleanSourceCodeVar = typedCodeVar(ArgType.BOOLEAN);
		SSAVar booleanSource = makeSsaVar(7, 0, ArgType.BOOLEAN, booleanSourceCodeVar);
		bindConst(booleanSource, 1, ArgType.BOOLEAN);
		booleanSourceCodeVar.setSsaVars(List.of(booleanSource));

		CodeVar spillCodeVar = typedCodeVar(ArgType.INT);
		FieldInfo spillField = FieldInfo.from(
				root, ClassInfo.fromType(root, stateType), "I$0", ArgType.INT);
		SSAVar firstSpill = makeSsaVar(4, 0, ArgType.INT, spillCodeVar);
		IndexInsnNode firstGet = new IndexInsnNode(InsnType.IGET, spillField, 0);
		firstGet.setResult(firstSpill.getAssign());
		SSAVar secondSpill = makeSsaVar(4, 1, ArgType.INT, spillCodeVar);
		IndexInsnNode secondGet = new IndexInsnNode(InsnType.IGET, spillField, 0);
		secondGet.setResult(secondSpill.getAssign());
		SSAVar spillPhiResult = makeSsaVar(4, 2, ArgType.INT, spillCodeVar);
		PhiInsn spillPhi = makePhi(spillPhiResult);
		bindPhiArg(spillPhi, firstSpill, 0);
		bindPhiArg(spillPhi, secondSpill, 1);
		spillCodeVar.setSsaVars(List.of(firstSpill, secondSpill, spillPhiResult));

		CodeVar booleanLoopCodeVar = new CodeVar();
		SSAVar booleanInput = makeSsaVar(14, 0, ArgType.UNKNOWN, booleanLoopCodeVar);
		bindMove(booleanInput, booleanSource);
		SSAVar booleanPhiResult = makeSsaVar(14, 1, ArgType.UNKNOWN, booleanLoopCodeVar);
		PhiInsn booleanPhi = makePhi(booleanPhiResult);
		SSAVar booleanLoopBack = makeSsaVar(14, 2, ArgType.UNKNOWN, booleanLoopCodeVar);

		CodeVar relayCodeVar = new CodeVar();
		SSAVar relayPhiResult = makeSsaVar(14, 3, ArgType.UNKNOWN, relayCodeVar);
		PhiInsn relayPhi = makePhi(relayPhiResult);
		SSAVar spillInput = makeSsaVar(14, 4, ArgType.UNKNOWN, relayCodeVar);
		bindMove(spillInput, spillPhiResult);
		SSAVar booleanRelay = makeSsaVar(14, 5, ArgType.UNKNOWN, relayCodeVar);
		bindMove(booleanRelay, booleanPhiResult);
		booleanRelay.getAssignInsn().add(AFlag.SYNTHETIC);

		bindMove(booleanLoopBack, relayPhiResult);
		booleanLoopBack.getAssignInsn().add(AFlag.SYNTHETIC);
		bindPhiArg(booleanPhi, booleanInput, 2);
		bindPhiArg(booleanPhi, booleanLoopBack, 3);
		bindPhiArg(relayPhi, booleanRelay, 4);
		bindPhiArg(relayPhi, spillInput, 5);
		new IfNode(
				IfOp.EQ, -1, booleanPhiResult.getAssign().duplicate(),
				LiteralArg.make(0, ArgType.UNKNOWN));

		List<SSAVar> booleanLoopGroup = List.of(booleanInput, booleanPhiResult, booleanLoopBack);
		List<SSAVar> relayGroup = List.of(relayPhiResult, spillInput, booleanRelay);
		booleanLoopCodeVar.setType(ArgType.BOOLEAN);
		booleanLoopCodeVar.setSsaVars(booleanLoopGroup);
		relayCodeVar.setSsaVars(relayGroup);
		Map<CodeVar, List<SSAVar>> groups = new IdentityHashMap<>();
		groups.put(booleanLoopCodeVar, booleanLoopGroup);
		groups.put(relayCodeVar, relayGroup);

		FinishTypeInference.LateCoroutineBooleanIntRelay relay =
				FinishTypeInference.collectLateCoroutineBooleanIntRelay(
						groups, Set.of(relayCodeVar));

		assertThat(relay).isNotNull();
		assertThat(relay.intInputs).containsExactly(spillInput.getAssignInsn());
	}
	...
	@Test
	void repairBooleanPhiRelaySplitAcrossCodeVariables() {
		CodeVar firstCodeVar = new CodeVar();
		RegisterArg constAssign = InsnArg.reg(0, ArgType.UNKNOWN);
		SSAVar constVar = new SSAVar(0, 0, constAssign);
		constVar.setCodeVar(firstCodeVar);
		InsnNode constInsn = new InsnNode(InsnType.CONST, 1);
		constInsn.setResult(constAssign);
		constInsn.addArg(LiteralArg.make(1, ArgType.UNKNOWN));

		RegisterArg phiAssign = InsnArg.reg(0, ArgType.UNKNOWN);
		SSAVar phiVar = new SSAVar(0, 1, phiAssign);
		phiVar.setCodeVar(firstCodeVar);

		CodeVar secondCodeVar = new CodeVar();
		RegisterArg relayAssign = InsnArg.reg(1, ArgType.UNKNOWN);
		SSAVar relayVar = new SSAVar(1, 0, relayAssign);
		relayVar.setCodeVar(secondCodeVar);
		InsnNode relayMove = new InsnNode(InsnType.MOVE, 1);
		relayMove.setResult(relayAssign);
		relayMove.addArg(phiAssign.duplicate());

		PhiInsn phiInsn = new PhiInsn(0, 2);
		phiInsn.setResult(phiAssign);
		phiInsn.bindArg(constAssign.duplicate(), new BlockNode(0, 0, 0));
		phiInsn.bindArg(relayAssign.duplicate(), new BlockNode(1, 0, 0));
		IfNode condition = new IfNode(
				IfOp.NE, -1, relayAssign.duplicate(), LiteralArg.make(0, ArgType.UNKNOWN));
		firstCodeVar.setSsaVars(List.of(constVar, phiVar));
		secondCodeVar.setSsaVars(List.of(relayVar));

		int repaired = FinishTypeInference.repairLateBooleanPhiRelayComponents(
				List.of(constVar, phiVar, relayVar));

		assertThat(repaired).isOne();
		assertThat(firstCodeVar.getType()).isEqualTo(ArgType.BOOLEAN);
		assertThat(secondCodeVar.getType()).isEqualTo(ArgType.BOOLEAN);
		assertThat(constInsn.getArg(0).getType()).isEqualTo(ArgType.BOOLEAN);
		assertThat(condition.getArg(1).getType()).isEqualTo(ArgType.BOOLEAN);
	}

	@Test
	void rejectBooleanPhiRelayWithNumericComparison() {
		CodeVar firstCodeVar = new CodeVar();
		RegisterArg constAssign = InsnArg.reg(0, ArgType.UNKNOWN);
		SSAVar constVar = new SSAVar(0, 0, constAssign);
		constVar.setCodeVar(firstCodeVar);
		...
		phiInsn.bindArg(constAssign.duplicate(), new BlockNode(0, 0, 0));
		phiInsn.bindArg(relayAssign.duplicate(), new BlockNode(1, 0, 0));
		new IfNode(IfOp.GT, -1, relayAssign.duplicate(), LiteralArg.make(0, ArgType.INT));
		firstCodeVar.setSsaVars(List.of(constVar, phiVar));
		secondCodeVar.setSsaVars(List.of(relayVar));
		...
		assertThat(firstCodeVar.getType()).isNull();
		assertThat(secondCodeVar.getType()).isNull();
	}

	@Test
	void convertLateBooleanArrayIndexToInteger() {
		CodeVar booleanCodeVar = new CodeVar();
		booleanCodeVar.setType(ArgType.BOOLEAN);
		...
		booleanVar.setCodeVar(booleanCodeVar);
		InsnNode constInsn = new InsnNode(InsnType.CONST, 1);
		constInsn.setResult(booleanAssign);
		constInsn.addArg(LiteralArg.litTrue());
		booleanCodeVar.setSsaVars(List.of(booleanVar));

		InsnNode arrayGet = new InsnNode(InsnType.AGET, 2);
		arrayGet.addArg(makeKnownSource(1, ArgType.array(ArgType.OBJECT)).duplicate());
		arrayGet.addArg(booleanAssign.duplicate());

		int repaired = FinishTypeInference.repairLateBooleanArrayIndexUses(List.of(booleanVar));

		assertThat(repaired).isOne();
		assertThat(arrayGet.getArg(1).isInsnWrap()).isTrue();
		assertThat(arrayGet.getArg(1).getType()).isEqualTo(ArgType.INT);
		assertThat(arrayGet.getArg(1).unwrap()).isInstanceOf(TernaryInsn.class);
	}

	@Test
	void repairLateBooleanLiteralNumericLoopIndexPhi() {
		CodeVar booleanCodeVar = typedCodeVar(ArgType.BOOLEAN);
		SSAVar booleanZero = makeSsaVar(9, 0, ArgType.BOOLEAN, booleanCodeVar);
		bindConst(booleanZero, 0, ArgType.BOOLEAN);
		booleanCodeVar.setSsaVars(List.of(booleanZero));

		CodeVar indexCodeVar = new CodeVar();
		SSAVar booleanEdge = makeSsaVar(3, 0, ArgType.UNKNOWN, indexCodeVar);
		bindMove(booleanEdge, booleanZero);

		SSAVar arithmeticEdge = makeSsaVar(3, 1, ArgType.UNKNOWN, indexCodeVar);
		new ArithNode(
				ArithOp.SUB,
				arithmeticEdge.getAssign(),
				makeKnownSource(7, ArgType.INT).duplicate(),
				LiteralArg.make(1, ArgType.INT));

		SSAVar phiResult = makeSsaVar(3, 2, ArgType.UNKNOWN, indexCodeVar);
		PhiInsn phi = makePhi(phiResult);
		bindPhiArg(phi, booleanEdge, 0);
		bindPhiArg(phi, arithmeticEdge, 1);
		indexCodeVar.setSsaVars(List.of(booleanEdge, arithmeticEdge, phiResult));

		CodeVar loopIndexCodeVar = typedCodeVar(ArgType.INT);
		SSAVar loopIndex = makeSsaVar(3, 3, ArgType.UNKNOWN, loopIndexCodeVar);
		RegisterArg phiUse = phiResult.getAssign().duplicate();
		phiResult.use(phiUse);
		InsnNode outputMove = new InsnNode(InsnType.MOVE, 1);
		outputMove.setResult(loopIndex.getAssign());
		outputMove.addArg(phiUse);
		outputMove.add(AFlag.SYNTHETIC);
		loopIndexCodeVar.setSsaVars(List.of(loopIndex));

		int repaired = FinishTypeInference.repairLateBooleanLiteralNumericPhiInputs(
				List.of(booleanZero, booleanEdge, arithmeticEdge, phiResult, loopIndex));

		assertThat(repaired).isOne();
		assertThat(indexCodeVar.getType()).isEqualTo(ArgType.INT);
		assertThat(booleanEdge.getAssignInsn().getArg(0)).isEqualTo(LiteralArg.make(0, ArgType.INT));
		assertThat(booleanCodeVar.getType()).isEqualTo(ArgType.BOOLEAN);
	}

	@Test
	void rejectDynamicBooleanNumericLoopIndexPhi() {
		RegisterArg dynamicBoolean = makeKnownSource(9, ArgType.BOOLEAN);
		CodeVar indexCodeVar = new CodeVar();
		SSAVar booleanEdge = makeSsaVar(3, 0, ArgType.UNKNOWN, indexCodeVar);
		InsnNode booleanMove = new InsnNode(InsnType.MOVE, 1);
		booleanMove.setResult(booleanEdge.getAssign());
		RegisterArg booleanUse = dynamicBoolean.duplicate();
		dynamicBoolean.getSVar().use(booleanUse);
		booleanMove.addArg(booleanUse);
		...
		int repaired = FinishTypeInference.repairLateBooleanLiteralNumericPhiInputs(
				List.of(booleanEdge, arithmeticEdge, phiResult, loopIndex));

		assertThat(repaired).isZero();
		assertThat(indexCodeVar.getType()).isNull();
		assertThat(booleanMove.getArg(0)).isSameAs(booleanUse);
	}
	...
	void removeOnlyTypeInferenceWarningsDuringFinalValidation() {
		...
		comments.add(CommentsLevel.WARN, "Type inference incomplete: some casts might be missing");
		...
		assertThat(comments.getComments().get(CommentsLevel.WARN))
				.containsExactlyInAnyOrder(
						"Type inference incomplete: some casts might be missing",
						"Code duplicated in 1 blocks");
		assertThat(comments.getComments().get(CommentsLevel.DEBUG)).containsExactly("debug detail");
		assertThat(ReportTypeInferenceWarnings.hasIncompleteFieldTypeWarning(comments)).isTrue();
		assertThat(ReportTypeInferenceWarnings.removeIncompleteFieldTypeWarning(comments)).isTrue();
		assertThat(comments.getComments().get(CommentsLevel.WARN))
	...
	@Test
	void ignoreStructuralMoveSourceWithoutInitializedCodeVar() {
		RegisterArg sourceAssign = InsnArg.reg(1, ArgType.UNKNOWN_OBJECT);
		new SSAVar(1, 0, sourceAssign); // deliberately not assigned to a CodeVar yet

		RegisterArg moveAssign = InsnArg.reg(0, ArgType.UNKNOWN_OBJECT);
		SSAVar moveVar = new SSAVar(0, 0, moveAssign);
		InsnNode moveInsn = new InsnNode(InsnType.MOVE, 1);
		moveInsn.setResult(moveAssign);
		RegisterArg sourceUse = sourceAssign.duplicate();
		moveInsn.addArg(sourceUse);

		RegisterArg siblingAssign = InsnArg.reg(0, ArgType.UNKNOWN_OBJECT);
		SSAVar siblingVar = new SSAVar(0, 1, siblingAssign);
		CodeVar mixedCodeVar = new CodeVar();
		moveVar.setCodeVar(mixedCodeVar);
		siblingVar.setCodeVar(mixedCodeVar);
		mixedCodeVar.setSsaVars(new ArrayList<>(List.of(moveVar, siblingVar)));
		Map<CodeVar, List<SSAVar>> groups = new IdentityHashMap<>();
		groups.put(mixedCodeVar, List.of(moveVar, siblingVar));

		assertThat(FinishTypeInference.splitStructuralReferenceMoveRootLifetimes(groups)).isZero();
		assertThat(FinishTypeInference.selectMoveSourceType(List.of(moveVar, siblingVar), mixedCodeVar)).isNull();
		RegisterArg thirdAssign = InsnArg.reg(0, ArgType.UNKNOWN_OBJECT);
		SSAVar thirdVar = new SSAVar(0, 2, thirdAssign);
		InsnNode thirdMove = new InsnNode(InsnType.MOVE, 1);
		thirdMove.setResult(thirdAssign);
		thirdMove.addArg(sourceAssign.duplicate());
		thirdVar.setCodeVar(mixedCodeVar);
		assertThat(FinishTypeInference.selectClosedReferenceMoveType(
				List.of(moveVar, siblingVar, thirdVar), mixedCodeVar)).isNull();
	}
	...
	@Test
	void splitClosedReferenceReceiverPhiFromMixedLifetime() {
		RootNode root = new RootNode(new JadxArgs());
		ArgType flowCollectorType = ArgType.object("kotlinx.coroutines.flow.FlowCollector");
		CodeVar firstSourceCodeVar = typedCodeVar(flowCollectorType);
		firstSourceCodeVar.setName("flowCollector");
		SSAVar firstSource = makeSsaVar(1, 0, flowCollectorType, firstSourceCodeVar);
		CodeVar secondSourceCodeVar = typedCodeVar(flowCollectorType);
		secondSourceCodeVar.setName("flowCollector");
		SSAVar secondSource = makeSsaVar(2, 0, flowCollectorType, secondSourceCodeVar);

		CodeVar mixedCodeVar = new CodeVar();
		mixedCodeVar.setName("exception");
		SSAVar firstInput = makeSsaVar(9, 0, ArgType.UNKNOWN_OBJECT, mixedCodeVar);
		bindMove(firstInput, firstSource);
		SSAVar secondInput = makeSsaVar(9, 1, ArgType.UNKNOWN_OBJECT, mixedCodeVar);
		bindMove(secondInput, secondSource);
		SSAVar receiverPhiResult = makeSsaVar(9, 2, ArgType.UNKNOWN_OBJECT, mixedCodeVar);
		PhiInsn receiverPhi = makePhi(receiverPhiResult);
		bindPhiArg(receiverPhi, firstInput, 0);
		bindPhiArg(receiverPhi, secondInput, 1);

		MethodInfo emitMethod = MethodInfo.fromDetails(
				root, ClassInfo.fromType(root, flowCollectorType), "emit", List.of(), ArgType.OBJECT);
		InvokeNode emit = new InvokeNode(emitMethod, InvokeType.INTERFACE, 1);
		RegisterArg receiverUse = receiverPhiResult.getAssign().duplicate();
		receiverPhiResult.use(receiverUse);
		emit.addArg(receiverUse);

		SSAVar exceptionValue = makeSsaVar(9, 3, ArgType.UNKNOWN_OBJECT, mixedCodeVar);
		IndexInsnNode exceptionConst = new IndexInsnNode(InsnType.CONST_STR, "exception", 0);
		exceptionConst.setResult(exceptionValue.getAssign());
		SSAVar mixedPhiResult = makeSsaVar(9, 4, ArgType.UNKNOWN_OBJECT, mixedCodeVar);
		PhiInsn mixedPhi = makePhi(mixedPhiResult);
		bindPhiArg(mixedPhi, receiverPhiResult, 2);
		bindPhiArg(mixedPhi, exceptionValue, 3);

		List<SSAVar> mixedGroup = List.of(
				firstInput, secondInput, receiverPhiResult, exceptionValue, mixedPhiResult);
		mixedCodeVar.setSsaVars(mixedGroup);
		firstSourceCodeVar.setSsaVars(List.of(firstSource));
		secondSourceCodeVar.setSsaVars(List.of(secondSource));
		Map<CodeVar, List<SSAVar>> groups = new IdentityHashMap<>();
		groups.put(mixedCodeVar, mixedGroup);

		assertThat(FinishTypeInference.splitClosedReferenceReceiverPhiLifetimes(groups)).isEqualTo(3);
		CodeVar receiverCodeVar = receiverPhiResult.getCodeVar();
		assertThat(receiverCodeVar).isNotSameAs(mixedCodeVar);
		assertThat(receiverCodeVar.getType()).isEqualTo(flowCollectorType);
		assertThat(receiverCodeVar.getName()).isEqualTo("flowCollector");
		assertThat(receiverCodeVar.getSsaVars()).containsExactly(firstInput, secondInput, receiverPhiResult);
		assertThat(mixedCodeVar.getSsaVars()).containsExactly(exceptionValue, mixedPhiResult);
		assertThat(((RegisterArg) emit.getInstanceArg()).getSVar().getCodeVar()).isSameAs(receiverCodeVar);
	}

	@Test
	void splitTerminalConstructorBeforeWideningReferencePhi() {
		RootNode root = new RootNode(new JadxArgs());
		root.loadClasses(List.of());
		root.initClassPath();
		ArgType exceptionType = ArgType.object("java.lang.IllegalArgumentException");
		ArgType functionType = ArgType.object("kotlin.jvm.functions.Function2");

		CodeVar functionCodeVar = typedCodeVar(functionType);
		SSAVar functionSource = makeSsaVar(1, 0, functionType, functionCodeVar);
		CodeVar mixedCodeVar = new CodeVar();
		mixedCodeVar.setName("exception");
		SSAVar exceptionValue = makeSsaVar(9, 0, exceptionType, mixedCodeVar);
		MethodInfo exceptionConstructor = MethodInfo.fromDetails(
				root, ClassInfo.fromType(root, exceptionType), "<init>", List.of(), ArgType.VOID);
		ConstructorInsn newException = new ConstructorInsn(
				exceptionConstructor, ConstructorInsn.CallType.CONSTRUCTOR);
		newException.setResult(exceptionValue.getAssign());

		MethodInfo errorConstructor = MethodInfo.fromDetails(
				root, ClassInfo.fromName(root, "test.Error"), "<init>",
				List.of(ArgType.THROWABLE), ArgType.VOID);
		ConstructorInsn newError = new ConstructorInsn(errorConstructor, ConstructorInsn.CallType.CONSTRUCTOR);
		RegisterArg throwableUse = exceptionValue.getAssign().duplicate();
		throwableUse.forceSetInitType(ArgType.THROWABLE);
		exceptionValue.use(throwableUse);
		newError.addArg(throwableUse);

		SSAVar functionInput = makeSsaVar(9, 1, ArgType.UNKNOWN_OBJECT, mixedCodeVar);
		bindMove(functionInput, functionSource);
		SSAVar mixedPhiResult = makeSsaVar(9, 2, ArgType.UNKNOWN_OBJECT, mixedCodeVar);
		PhiInsn mixedPhi = makePhi(mixedPhiResult);
		bindPhiArg(mixedPhi, exceptionValue, 0);
		bindPhiArg(mixedPhi, functionInput, 1);
		SSAVar internalMove = makeSsaVar(9, 3, ArgType.UNKNOWN_OBJECT, mixedCodeVar);
		bindMove(internalMove, mixedPhiResult);

		List<SSAVar> mixedGroup = List.of(exceptionValue, functionInput, mixedPhiResult, internalMove);
		mixedCodeVar.setSsaVars(mixedGroup);
		functionCodeVar.setSsaVars(List.of(functionSource));
		Map<CodeVar, List<SSAVar>> groups = new IdentityHashMap<>();
		groups.put(mixedCodeVar, mixedGroup);

		assertThat(FinishTypeInference.splitTerminalReferenceConstructorLifetimes(
				groups, root.getTypeCompare())).isOne();
		assertThat(exceptionValue.getCodeVar()).isNotSameAs(mixedCodeVar);
		assertThat(exceptionValue.getCodeVar().getType()).isEqualTo(exceptionType);
		assertThat(mixedCodeVar.getSsaVars()).containsExactly(functionInput, mixedPhiResult, internalMove);
		assertThat(FinishTypeInference.isReferenceOnlyPhiFlow(
				mixedCodeVar.getSsaVars(), mixedCodeVar)).isTrue();
		FinishTypeInference.repairLateReferenceOnlyPhiFlows(null, mixedCodeVar.getSsaVars());
		assertThat(mixedCodeVar.getType()).isEqualTo(ArgType.OBJECT);
	}

	@Test
	void restoreWrappedLongIdentityAfterCoroutinePhiJoin() {
		CodeVar firstLongCodeVar = typedCodeVar(ArgType.LONG);
		SSAVar firstLong = makeSsaVar(1, 0, ArgType.LONG, firstLongCodeVar);
		CodeVar secondLongCodeVar = typedCodeVar(ArgType.LONG);
		SSAVar secondLong = makeSsaVar(2, 0, ArgType.LONG, secondLongCodeVar);

		CodeVar sharedCodeVar = new CodeVar();
		sharedCodeVar.setDeclared(true);
		SSAVar firstInput = makeSsaVar(13, 0, ArgType.UNKNOWN, sharedCodeVar);
		bindMove(firstInput, firstLong);
		SSAVar secondInput = makeSsaVar(13, 1, ArgType.UNKNOWN, sharedCodeVar);
		bindMove(secondInput, secondLong);
		SSAVar firstPhiResult = makeSsaVar(13, 2, ArgType.UNKNOWN, sharedCodeVar);
		PhiInsn firstPhi = makePhi(firstPhiResult);
		bindPhiArg(firstPhi, firstInput, 0);
		bindPhiArg(firstPhi, secondInput, 1);
		SSAVar zeroInput = makeSsaVar(13, 3, ArgType.UNKNOWN, sharedCodeVar);
		bindConst(zeroInput, 0, ArgType.UNKNOWN);
		SSAVar loopPhiResult = makeSsaVar(13, 4, ArgType.UNKNOWN, sharedCodeVar);
		PhiInsn loopPhi = makePhi(loopPhiResult);
		bindPhiArg(loopPhi, firstPhiResult, 2);
		bindPhiArg(loopPhi, zeroInput, 3);

		RegisterArg conditionSource = firstPhiResult.getAssign().duplicate();
		IfNode condition = new IfNode(IfOp.EQ, -1, conditionSource, LiteralArg.litTrue());
		TernaryInsn longProjection = new TernaryInsn(
				IfCondition.fromIfNode(condition), null,
				LiteralArg.make(1, ArgType.LONG), LiteralArg.make(0, ArgType.LONG));
		InsnNode fieldWrite = new InsnNode(InsnType.IPUT, 1);
		fieldWrite.addArg(InsnArg.wrapInsnIntoArg(longProjection));

		List<SSAVar> sharedGroup = List.of(
				firstInput, secondInput, firstPhiResult, zeroInput, loopPhiResult);
		sharedCodeVar.setSsaVars(sharedGroup);
		firstLongCodeVar.setSsaVars(List.of(firstLong));
		secondLongCodeVar.setSsaVars(List.of(secondLong));
		List<SSAVar> allVars = List.of(
				firstInput, secondInput, firstPhiResult, zeroInput, loopPhiResult,
				firstLong, secondLong);

		assertThat(FinishTypeInference.repairLateCoroutineLongIdentityFlows(
				null, allVars, List.of(fieldWrite))).isOne();
		assertThat(sharedCodeVar.getType()).isEqualTo(ArgType.LONG);
		assertThat(sharedGroup).allMatch(var -> ArgType.LONG.equals(var.getTypeInfo().getType()));
		assertThat(fieldWrite.getArg(0).isRegister()).isTrue();
		assertThat(((RegisterArg) fieldWrite.getArg(0)).getSVar()).isSameAs(firstPhiResult);
		assertThat(firstPhiResult.getUseList())
				.noneMatch(use -> use.getParentInsn() instanceof IfNode);
	}
	...
	@Test
	void splitReferenceLoopPhiFromPrimitiveExceptionPhi() {
		ExceptionReferencePrimitivePhiFixture fixture = new ExceptionReferencePrimitivePhiFixture();

		assertThat(FinishTypeInference.repairExceptionReferencePrimitivePhiLifetimes(
				fixture.groups, input -> input == fixture.exceptionReferenceInput)).isOne();
		assertThat(fixture.firstReferenceRoot.getCodeVar())
				.isSameAs(fixture.secondReferenceRoot.getCodeVar())
				.isSameAs(fixture.referencePhiResult.getCodeVar());
		assertThat(fixture.firstReferenceRoot.getCodeVar().getType()).isEqualTo(fixture.referenceType);
		assertThat(fixture.firstReferenceRoot.getCodeVar().getSsaVars()).containsExactly(
				fixture.firstReferenceRoot, fixture.secondReferenceRoot, fixture.referencePhiResult);
		assertThat(fixture.primitiveRoot.getCodeVar()).isSameAs(fixture.mixedPhiResult.getCodeVar());
		assertThat(fixture.primitiveRoot.getCodeVar().getType()).isEqualTo(ArgType.LONG);
		assertThat(fixture.primitiveRoot.getCodeVar().getSsaVars()).containsExactly(
				fixture.primitiveRoot, fixture.mixedPhiResult);
	}

	@Test
	void rejectReferencePrimitivePhiWithoutExceptionSplitterInput() {
		ExceptionReferencePrimitivePhiFixture fixture = new ExceptionReferencePrimitivePhiFixture();

		assertThat(FinishTypeInference.repairExceptionReferencePrimitivePhiLifetimes(
				fixture.groups, input -> false)).isZero();
		assertThat(fixture.firstReferenceRoot.getCodeVar()).isSameAs(fixture.mixedPhiResult.getCodeVar());
		assertThat(fixture.firstReferenceRoot.getCodeVar().getType()).isNull();
	}

	@Test
	void splitReferenceLoopPhiWithClosedPrimitiveTail() {
		ExceptionReferencePrimitivePhiFixture fixture = new ExceptionReferencePrimitivePhiFixture(true);
		...
		assertThat(fixture.primitiveRoot.getCodeVar().getSsaVars()).containsExactly(
				fixture.primitiveRoot, fixture.mixedPhiResult,
				fixture.tailPrimitiveRoot, fixture.tailPhiResult);
	}

	@Test
	void splitReferenceLoopPhiAfterPrimitiveTypeWasSelected() {
		ExceptionReferencePrimitivePhiFixture fixture = new ExceptionReferencePrimitivePhiFixture(true);
		fixture.primitiveRoot.getCodeVar().setType(ArgType.LONG);
	...
	@Test
	void splitAllIndependentReferencePrimitiveExceptionGroups() {
		ExceptionReferencePrimitivePhiFixture first = new ExceptionReferencePrimitivePhiFixture();
		ExceptionReferencePrimitivePhiFixture second = new ExceptionReferencePrimitivePhiFixture(true);
		Map<CodeVar, List<SSAVar>> groups = new IdentityHashMap<>(first.groups);
		groups.putAll(second.groups);

		assertThat(FinishTypeInference.repairExceptionReferencePrimitivePhiLifetimes(
				groups,
				input -> input == first.exceptionReferenceInput
						|| input == second.exceptionReferenceInput))
								.isEqualTo(2);
		assertThat(first.firstReferenceRoot.getCodeVar().getType()).isEqualTo(first.referenceType);
		assertThat(second.firstReferenceRoot.getCodeVar().getType()).isEqualTo(second.referenceType);
	}
	...
	private static final class ExceptionReferencePrimitivePhiFixture {
		private final ArgType referenceType = ArgType.object("kotlinx.coroutines.CoroutineScope");
		private final Map<CodeVar, List<SSAVar>> groups = new IdentityHashMap<>();
		private final SSAVar firstReferenceRoot;
		private final SSAVar secondReferenceRoot;
		private final SSAVar referencePhiResult;
		private final SSAVar primitiveRoot;
		private final SSAVar mixedPhiResult;
		private final SSAVar tailPrimitiveRoot;
		private final SSAVar tailPhiResult;
		private final RegisterArg exceptionReferenceInput;

		private ExceptionReferencePrimitivePhiFixture() {
			this(false);
		}

		private ExceptionReferencePrimitivePhiFixture(boolean addPrimitiveTail) {
			CodeVar firstSourceCodeVar = typedCodeVar(referenceType);
			SSAVar firstSource = makeSsaVar(1, 0, referenceType, firstSourceCodeVar);
			CodeVar secondSourceCodeVar = typedCodeVar(referenceType);
			SSAVar secondSource = makeSsaVar(2, 0, referenceType, secondSourceCodeVar);
			CodeVar longSourceCodeVar = typedCodeVar(ArgType.LONG);
			SSAVar longSource = makeSsaVar(3, 0, ArgType.LONG, longSourceCodeVar);

			CodeVar mixedCodeVar = new CodeVar();
			firstReferenceRoot = makeSsaVar(8, 3, ArgType.UNKNOWN_OBJECT, mixedCodeVar);
			bindMove(firstReferenceRoot, firstSource);
			secondReferenceRoot = makeSsaVar(8, 12, ArgType.UNKNOWN_OBJECT, mixedCodeVar);
			bindMove(secondReferenceRoot, secondSource);
			referencePhiResult = makeSsaVar(8, 5, ArgType.UNKNOWN, mixedCodeVar);
			PhiInsn referencePhi = makePhi(referencePhiResult);
			bindPhiArg(referencePhi, firstReferenceRoot, 10);
			bindPhiArg(referencePhi, secondReferenceRoot, 11);

			primitiveRoot = makeSsaVar(8, 14, ArgType.UNKNOWN, mixedCodeVar);
			bindMove(primitiveRoot, longSource);
			mixedPhiResult = makeSsaVar(8, 1, ArgType.UNKNOWN, mixedCodeVar);
			PhiInsn mixedPhi = makePhi(mixedPhiResult);
			bindPhiArg(mixedPhi, referencePhiResult, 12);
			exceptionReferenceInput = mixedPhi.getArg(0);
			bindPhiArg(mixedPhi, primitiveRoot, 13);

			List<SSAVar> mixedGroup = new java.util.ArrayList<>(List.of(
					firstReferenceRoot, secondReferenceRoot, referencePhiResult,
					primitiveRoot, mixedPhiResult));
			if (addPrimitiveTail) {
				tailPrimitiveRoot = makeSsaVar(8, 7, ArgType.UNKNOWN, mixedCodeVar);
				bindMove(tailPrimitiveRoot, longSource);
				tailPhiResult = makeSsaVar(8, 4, ArgType.UNKNOWN, mixedCodeVar);
				PhiInsn tailPhi = makePhi(tailPhiResult);
				bindPhiArg(tailPhi, mixedPhiResult, 14);
				bindPhiArg(tailPhi, tailPrimitiveRoot, 15);
				mixedGroup.add(tailPrimitiveRoot);
				mixedGroup.add(tailPhiResult);
			} else {
				tailPrimitiveRoot = null;
				tailPhiResult = null;
			}
			mixedCodeVar.setSsaVars(mixedGroup);
			firstSourceCodeVar.setSsaVars(List.of(firstSource));
			...
			longSourceCodeVar.setSsaVars(List.of(longSource));
			groups.put(mixedCodeVar, mixedGroup);
			groups.put(firstSourceCodeVar, firstSourceCodeVar.getSsaVars());
			groups.put(secondSourceCodeVar, secondSourceCodeVar.getSsaVars());
			groups.put(longSourceCodeVar, longSourceCodeVar.getSsaVars());
		}
	}
