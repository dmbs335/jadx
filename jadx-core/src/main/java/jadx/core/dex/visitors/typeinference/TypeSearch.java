...
public class TypeSearch {
	...
	private void addNarrowTypes(Set<ITypeBound> bounds, Set<ArgType> candidateTypes, ArgType type) {
		if (type.isTypeKnown()) {
			if (type.isObject()) {
				...
				List<ArgType> impList = mth.root().getClsp().getImplementationTypes(type.getObject());
				addCandidateTypes(bounds, candidateTypes, impList);
			}
		} else {
			addCandidateTypes(bounds, candidateTypes, expandUnknownType(type));
		}
	...
	private void addObjectTypes(
		...
		for (String objectName : objectNames) {
			...
			if (objectType == null) {
				objectType = ArgType.object(objectName);
			}
			if (addCandidateType(bounds, candidateTypes, objectType)) {
				return;
			}
		}
	}
