...
public class TernaryMod extends AbstractRegionVisitor implements IRegionIterativeVisitor {
	...
	private static boolean makeTernaryInsn(MethodNode mth, IfRegion ifRegion) {
		...
		// Moving both branch values into a ternary would empty shared value blocks.
		// Preserve them when sibling paths with observable effects still enter both blocks.
		if (hasExternalEffectPredecessor(tb, conditionBlocks)
				&& hasExternalEffectPredecessor(eb, conditionBlocks)) {
			return false;
		}
	...
	private static boolean hasExternalEffectPredecessor(
			BlockNode branchBlock, List<BlockNode> conditionBlocks) {
		return branchBlock.getPredecessors().stream()
				.map(pred -> BlockUtils.followEmptyPath(pred, true))
				.filter(pred -> !conditionBlocks.contains(pred))
				.flatMap(pred -> pred.getInstructions().stream())
				.anyMatch(insn -> insn.getType() != InsnType.IF
						&& insn.getType() != InsnType.SWITCH
						&& !insn.canReorder());
	}
	...
	private static BlockNode getTernaryInsnBlock(MethodNode mth, IContainer thenRegion) {
		...
		if (block != null) {
			if (block.getInstructions().stream().anyMatch(insn -> insn.contains(AFlag.DONT_GENERATE))) {
				// The instruction is already owned by another structured region (for example a
				// for-loop increment). Wrapping it into a live ternary would make generation
				// requirements contradictory and can also consume the structured expression.
				return null;
			}
