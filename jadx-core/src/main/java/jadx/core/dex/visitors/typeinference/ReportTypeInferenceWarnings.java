...
public final class ReportTypeInferenceWarnings extends AbstractVisitor {
	...
	public void visit(MethodNode mth) {
		...
		boolean incompleteFieldType = hasIncompleteFieldTypeWarning(commentsAttr);
		if (!removeTypeInferenceWarnings(commentsAttr) && !incompleteFieldType) {
			return;
		}
		...
		FinishTypeInference.repairLateCoroutineLongIdentityFlows(mth);
		FixTypesVisitor.repairLateIntBooleanPhiUses(mth);
		FinishTypeInference.repairLateBooleanIntLoopFlows(mth);
		FinishTypeInference.repairLateBooleanLiteralNumericPhiInputs(mth.getSVars());
		FinishTypeInference.repairLateBooleanPhiRelayComponents(mth.getSVars());
		FinishTypeInference.repairLateBooleanArrayIndexUses(mth.getSVars());
		...
		if (incompleteFieldType && areMarkedFieldTypeConflictsResolved(mth)) {
			removeIncompleteFieldTypeWarning(commentsAttr);
		}
	...
	private static boolean areMarkedFieldTypeConflictsResolved(MethodNode mth) {
		boolean found = false;
		for (SSAVar var : mth.getSVars()) {
			if (var.getAssignInsn() == null
					|| !var.getAssignInsn().contains(AType.INCOMPLETE_FIELD_TYPE_CONFLICT)) {
				continue;
			}
			found = true;
			if (!FixTypesVisitor.isOnlyCompatibleFieldTypeConflict(var, mth.root().getTypeCompare())) {
				return false;
			}
		}
		return found;
	...
	static boolean hasIncompleteFieldTypeWarning(JadxCommentsAttr commentsAttr) {
		Set<String> warnings = commentsAttr.getComments().get(CommentsLevel.WARN);
		return warnings != null
				&& warnings.stream().anyMatch(warning -> warning.startsWith("Type inference incomplete:"));
	}

	static boolean removeIncompleteFieldTypeWarning(JadxCommentsAttr commentsAttr) {
		Map<CommentsLevel, Set<String>> comments = commentsAttr.getComments();
		Set<String> warnings = comments.get(CommentsLevel.WARN);
		if (warnings == null) {
			return false;
		}
		boolean removed = warnings.removeIf(warning -> warning.startsWith("Type inference incomplete:"));
		if (warnings.isEmpty()) {
