...
public class SignatureProcessor extends AbstractVisitor {
		...
		for (ArgType typeParameter : typeParameters) {
			...
			for (int boundIndex = 1; boundIndex < bounds.size(); boundIndex++) {
				...
				if (bound.isGenericType()) {
					if (javaBounds == null) {
						javaBounds = new ArrayList<>(boundIndex);
						for (int previousIndex = 0; previousIndex < boundIndex; previousIndex++) {
							javaBounds.add(bounds.get(previousIndex));
						}
					}
	...
	private boolean validateAndApplyTypes(MethodNode mth, SignatureParser sp, ArgType retType, List<ArgType> argTypes) {
		try {
			...
			mth.updateTypes(checkedArgTypes, retType);
			return true;
		} catch (Exception e) {
