...
public class JadxCLIArgs implements IJadxConfig {
	...
	@JadxConfigExclude
	@Parameter(
			names = { "--content-store-import-dir" },
			description = "ingest an existing JADX output directory without decompiling"
	)
	protected String contentStoreImportDir;
	...
	@JadxConfigExclude
	@Parameter(
			names = { "--content-store-retain-runs" },
			description = "delete older complete runs and orphan objects, keeping this many recent runs per application"
	)
	protected int contentStoreRetainRuns;
	...
	public void verify() {
		...
		int storeCommandCount = (contentStoreSearch == null ? 0 : 1)
				+ (contentStoreImportDir == null ? 0 : 1)
				+ (contentStoreCompact ? 1 : 0)
				+ (contentStoreRetainRuns > 0 ? 1 : 0)
		...
		if (contentStoreRetainRuns < 0) {
			throw new JadxArgsValidateException("Content-store retained run count can't be negative");
		}
		...
		if (contentStoreImportDir != null && files.isEmpty()) {
			throw new JadxArgsValidateException("Content-store import requires at least one input application file");
		}
	...
	public String getContentStoreImportDir() {
		return contentStoreImportDir;
	}
	...
	public int getContentStoreRetainRuns() {
		return contentStoreRetainRuns;
	}
	...
	public boolean hasContentStoreCommand() {
		return contentStoreSearch != null || contentStoreImportDir != null || contentStoreCompact || contentStoreStats
				|| contentStoreRetainRuns > 0 || contentStoreMaterializeRun > 0;
	}
